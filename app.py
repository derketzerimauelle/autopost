...
class App:
    ...
    def create_post(self, post):
        ...
        # Seleccionar categoría usando Selenium en lugar de pyautogui
        category_dropdown = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//label[@aria-label='" + self.marketplace_options["labels"]["Category"] +  "']"))
        )
        category_dropdown.click()
        sleep(self.time_to_sleep)

        # Seleccionar opción de categoría con base en el índice o nombre
        category_option = WebDriverWait(self.driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//div[@role='menu']//span[contains(text(), '" + post[3] + "')]"))
        )
        category_option.click()
        sleep(self.time_to_sleep)

        # Continuar con el resto de la creación del post
